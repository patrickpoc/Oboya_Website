    ...
    <section className="bg-white py-[var(--section-y)]">
      <Container>
        <motion.div
          ...
          variants={fadeInUp}
          className="flex flex-col items-center gap-12 md:gap-14"
        >
          ...
          <ul className="flex w-full max-w-5xl flex-wrap items-center justify-center gap-x-10 gap-y-10 md:gap-x-14 lg:gap-x-16">
            {data.logos.map((logo) => {
              const content = logo.image ? (
                <Image
                  ...
                  width={200}
                  height={80}
                  className="h-12 w-auto max-w-[11rem] object-contain md:h-14 md:max-w-[13rem]"
                />
              ) : (
              ...
              return (
                <li key={logo.id} className="flex h-16 items-center justify-center md:h-20">
                  {logo.href ? (
